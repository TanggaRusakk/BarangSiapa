<x-dashboard-layout>
    ...
    <div class="row g-4 mb-4">
        <div class="col-12 col-lg-4">
            <div class="card p-4">
                <h3 class="fw-bold mb-3 text-white">Profile</h3>
        ...
        <div class="col-12 col-lg-8">
            <div class="card p-4">
                <h3 class="fw-bold mb-3 text-white">Recent Orders</h3>
                @if($orders->isEmpty())
                        ...
                        @foreach($orders as $order)
                            <div class="p-3 rounded" style="background: rgba(106,56,194,0.1);" x-data="{ expanded: false }">
                                <div class="d-flex justify-content-between align-items-center cursor-pointer" @click="expanded = !expanded" style="cursor:pointer;">
                                    <div>
                                        <div class="fw-bold text-white">Order #{{ $order->id }}</div>
                                        <div class="small text-secondary">{{ $order->created_at ? $order->created_at->format('d M Y H:i') : '—' }}</div>
                                    </div>
                                    <div class="d-flex align-items-center gap-3">
                                        <div class="text-end">
                                            <div class="fw-bold text-white">Rp{{ number_format($order->total_amount ?? $order->order_total_amount ?? 0, 0) }}</div>
                                        ...
                                        <div class="d-flex flex-column gap-2">
                                            @foreach($order->orderItems as $oi)
                                                <div class="d-flex justify-content-between align-items-center p-2 rounded" style="background: rgba(255,255,255,0.05);">
                                                    ...
                                                    <div class="text-white">Rp{{ number_format($oi->order_item_price ?? $oi->price ?? 0, 0) }}</div>
                                                </div>
                                            @endforeach
                                        </div>
                                    @else
                ...
                <h3 class="fw-bold mb-3 text-white">Recent Reviews</h3>
                @if($reviews->isEmpty())
                    ...
                    <div class="d-flex flex-column gap-2">
                        @foreach($reviews as $rev)
                            <div class="p-3 rounded" style="background: rgba(106,56,194,0.1);">
                                <div class="fw-bold text-white">{{ optional($rev->item)->item_name ?? '—' }}</div>
                ...
                <h3 class="fw-bold mb-3 text-white">Recent Payments</h3>
                @if($payments->isEmpty())
                    ...
                    <div class="d-flex flex-column gap-2">
                        @foreach($payments as $p)
                            <div class="d-flex justify-content-between align-items-center p-3 rounded" style="background: rgba(106,56,194,0.1);">
                                <div>
                                    <div class="fw-bold text-white">Payment #{{ $p->id }}</div>
                                    <div class="small text-secondary">Method: {{ $p->payment_method ?? '—' }}</div>
                                </div>
                                <div class="text-end">
                                    <div class="fw-bold text-white">Rp{{ number_format($p->payment_total_amount ?? $p->amount ?? 0, 0) }}</div>
